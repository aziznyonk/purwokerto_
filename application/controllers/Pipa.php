...
class Pipa extends BaseController 
{
    ...
    public function index()
    {
        if($this->isAdmin() == TRUE)
        {
			...
			$data['pipaRecords'] = $this->pipa_model->pipaListing();
			$this->loadViews('pipa/pipa', $this->global, $data, null);
	...
	public function pipaRencana()
	{
		if($this->isAdmin() == TRUE)
        {
			...
			$data['pipaRecords'] = $this->pipa_model->pipaRencanaListing();
	...
	public function details_pipa()
	{
		...
		if($this->isAdmin() == TRUE)
        {
			...
			$this->global['title'] 	= 'Pipa';
			$data['pipa']			= 'Pipa';
			$data['pipaRecords'] 	= $this->pipa_model->pipaListing_details($id);
	...
	public function details_pipaRencana()
	{
		...
		if($this->isAdmin() == TRUE)
        {
			...
			$data['pipaRecords'] 	= $this->pipa_model->pipaRencanaListing_details($id);
			$this->loadViews('pipa/pipa_details', $this->global, $data, null);
	...
	public function save_pipa()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$id = $this->input->post('ID');
			$result = $this->pipa_model->pipaRencanaListing_save($id);
			if($result){
